(function(root, factory) {
  ...
  function traverseBranchIdAndWidth(node, branch) {
    node.branch = branch;
    node.depthWidth = depthMaxSize[node.depth];
    if (node.children) {
      node.children.forEach(function(d) {
        traverseBranchIdAndWidth(d, branch);
      });
    }
  }
  ...
  assign(Markmap.prototype, {
    ...
    layout: function(state) {
      ...
      // Normalize
      var ratio =
        (state.nodeHeight + state.spacingVertical) /
        traverseMinDistance(state.root);
      ...
      // traverseLabelWidth(state.root, 0);
    ...
    renderers: {
      ...
      basic: function(source, nodes, links) {
        ...
        nodeUpdate.select("text").attr("x", 10).style("fill-opacity", 1);
        ...
        nodeExit.select("rect").attr("x", function(d) {
          return d.depthWidth;
        }).attr("width", 0);
        ...
        // Update the links…
        var link = svg.selectAll("path.markmap-link").data(links, function(d) {
          return d.target.id;
        });
        ...
        link.transition().duration(state.duration).attr("d", function(d) {
